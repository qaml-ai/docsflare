...
const root = process.env.DOCSFLARE_CONTENT_DIR ? path.resolve(process.env.DOCSFLARE_CONTENT_DIR) : projectRoot;
...
function readConfig(): { config: MintConfig; filename: string } {
  ...
  return { config: { name: path.basename(root), navigation: [] }, filename: "generated" };
}
...
async function main() {
  ...
  console.log(`Built ${pages.length} page(s) from ${filename}.`);
}
